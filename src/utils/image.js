...
/**
 * JSTローカル時間の YYYY-MM-DD 文字列を出力する標準フォーマッタ
 */
export function formatDateToLocalYYYYMMDD(dateObj) {
  if (!dateObj) return '';
...
/**
 * 画像ファイルから撮影日時 (EXIF Tag 0x9003 / 0x9004 / 0x0132 または ファイル名) を高精度に抽出する関数
 * - 2MB ヘッダーパースによりスマホカメラの大きな埋め込みサムネイルによる打ち切りを防止
 * - HEIC / 大文字拡張子 (.JPG) / MIMEタイプ空文字等にも柔軟対応
 * - EXIFが消去された画像でもファイル名 (IMG_20241103_...等) から日時を自動補完
...
export function extractPhotoDate(file) {
  return new Promise((resolve) => {
    ...
    const fallback = () => {
      // ファイル名からの日付パターン判定 (例: IMG_20241103_184512.jpg, 2025-01-15_photo.png 等)
      const name = file.name || '';
      const nameMatch = name.match(/(?:19|20)\d{2}[:\/\.-]?(?:0[1-9]|1[0-2])[:\/\.-]?(?:0[1-9]|[12]\d|3[01])/);
      if (nameMatch) {
        const raw = nameMatch[0].replace(/[:\/\.]/g, '');
        if (raw.length === 8) {
    ...
    // 🌟 2MB (2097152 bytes) へ拡張し、大きなAPP1ヘッダーでも読み切れを防ぐ
    ...
    reader.onload = (e) => {
      try {
          ...
          if (subIFDOffset + 2 <= length) {
            ...
            for (let i = 0; i < subEntriesCount; i++) {
              ...
              if (tag === 0x9003 || tag === 0x9004) {
                targetDateStrOffset = view.getUint32(entryOffset + 8, isLittleEndian);
                break;
              }
            }
          }
        ...
        if (targetDateStrOffset === 0 && dateTimeIFD0Offset !== 0) {
          targetDateStrOffset = dateTimeIFD0Offset;
        }

        if (targetDateStrOffset === 0) { fallback(); return; }

        const dateStrOffset = tiffOffset + targetDateStrOffset;
...
/**
 * 撮影日時の Date オブジェクト (JST ローカル正午 12:00:00 固定) を抽出する非同期関数
 */
  ...
  return new Promise((resolve) => {
    extractPhotoDate(file).then(dateStr => {
      ...
      if (file.lastModified) {
        ...
        if (!isNaN(d.getTime())) {
          resolve(new Date(d.getFullYear(), d.getMonth(), d.getDate(), 12, 0, 0));
          return;
        }
      }
      resolve(null);
