  ...
  try {
    ...
    if (poolLog && poolLog.imageUrls && poolLog.imageUrls.length > 0) {
      ...
      for (let i = 0; i < poolLog.imageUrls.length; i++) {
        ...
        let previewUrl = (blob instanceof Blob) ? URL.createObjectURL(blob) : '';
        if (!previewUrl && meta.base64) {
          previewUrl = `data:${meta.mimeType || 'image/jpeg'};base64,${meta.base64}`;
        }
...
document.addEventListener('click', async (e) => {
  ...
  // 🌟 一括インポート画面上の「✕」ボタン（グループからの削除／プールからの削除）
...
document.addEventListener('sync-completed', () => {
  ...
  const restoredTheme = localStorage.getItem('sella_theme');
  if (restoredTheme) {
    setTheme(restoredTheme);
  }

  const apiKeyInput = document.getElementById('gemini-api-key');
  const restoredKey = localStorage.getItem('gemini_api_key');
  if (apiKeyInput && restoredKey && apiKeyInput.value !== restoredKey) {
    apiKeyInput.value = restoredKey;
  }
