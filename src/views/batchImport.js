...
/**
 * サムネイル画像の URL を安全に取得するヘルパー関数
 */
function getSafeImgSrc(item) {
  if (!item) return '';
  if (item.previewUrl && typeof item.previewUrl === 'string' && item.previewUrl.trim() !== '') {
...
export function renderBatchGroupsUI() {
  ...
  if (state.ungroupedImages.length > 0) {
      ...
      <div style="display: flex; gap: 10px; flex-wrap: wrap; min-height: 40px; margin-top: 10px;" class="thumbs-scroll-container">
        ${state.ungroupedImages.map((item, idx) => {
          ...
          <div class="draggable-thumb" draggable="true" data-source-type="pool" data-idx="${idx}"
               style="position:relative; width:90px; height:90px; border-radius:8px; overflow:visible; border:1px solid var(--border-color); box-shadow: none; box-sizing: border-box; touch-action: none; flex-shrink: 0;">
            <img src="${imgSrc}" 
                 data-base64="${item.base64 || ''}" 
                 data-mimetype="${item.mimeType || 'image/jpeg'}"
                 onerror="if(this.dataset.base64){this.src='data:'+(this.dataset.mimetype||'image/jpeg')+';base64,'+this.dataset.base64; this.onerror=null;}"
            ...
            <button type="button" class="btn-ungrouped-remove" data-idx="${idx}" title="削除"
                    style="position:absolute; top:-6px; right:-6px; background:#ef4444; color:#fff; border:2px solid #fff; border-radius:50%; width:24px; height:22px; font-size:12px; font-weight:bold; cursor:pointer; z-index:10; display:flex; align-items:center; justify-content:center; box-shadow:0 2px 6px rgba(0,0,0,0.4);">✕</button>
          </div>
        `}).join('')}
      </div>
    `;
    ...
    ungroupedHTML = `
      <div id="ungrouped-pool-container" style="position: fixed; bottom: 12px; left: 12px; right: 12px; max-width: 720px; margin: 0 auto; z-index: 100; background: var(--card-bg); border: 2px dashed var(--accent-color); border-radius: 12px; padding: 12px 14px; box-shadow: 0 8px 24px rgba(0,0,0,0.6); backdrop-filter: blur(12px); box-sizing: border-box;">
        <div style="display: flex; flex-wrap: wrap; justify-content: space-between; align-items: center; gap: 8px; width: 100%;">
          <div style="display: flex; align-items: center; gap: 6px; min-width: 0; flex: 1;">
            <button type="button" id="btn-toggle-pool-collapse" class="btn-secondary" style="font-size: 0.75rem; padding: 3px 8px; flex-shrink: 0;">${state.isPoolCollapsed ? '▶ 展開' : '▼ 畳む'}</button>
            <span style="font-weight: bold; color: var(--text-main); font-size: 0.85rem; white-space: nowrap; word-break: keep-all; overflow: hidden; text-overflow: ellipsis;">📂 未所属プール (${state.ungroupedImages.length}枚)</span>
          </div>
          <button type="button" id="btn-create-group-from-ungrouped" class="btn-secondary" style="font-size: 0.75rem; padding: 4px 10px; flex-shrink: 0; margin-left: auto;">✨ グループ作成</button>
        </div>
        ${thumbs}
      </div>
    `;
  ...
  const groupsHTML = state.batchGroups.map((group, gIdx) => {
    ...
    const thumbsHTML = group.map((item, iIdx) => {
      ...
      <div class="draggable-thumb" draggable="true" data-source-type="group" data-gidx="${gIdx}" data-iidx="${iIdx}"
           style="position:relative; width:90px; height:90px; border-radius:8px; overflow:visible; border: ${iIdx === 0 ? '3px solid var(--accent-color)' : '1px solid var(--border-color)'}; box-shadow: ${iIdx === 0 ? '0 0 10px rgba(var(--accent-color-rgb, 16, 185, 129), 0.3)' : 'none'}; box-sizing: border-box; touch-action: none; flex-shrink: 0;">
        <img src="${imgSrc}" 
             data-base64="${item.base64 || ''}" 
             data-mimetype="${item.mimeType || 'image/jpeg'}"
             onerror="if(this.dataset.base64){this.src='data:'+(this.dataset.mimetype||'image/jpeg')+';base64,'+this.dataset.base64; this.onerror=null;}"
...
/**
 * 🌟 一括インポートUIの操作イベントハンドラー（✕ボタン・プール操作等）
 */
if (typeof window !== 'undefined' && !window.__batchImportListenersAttached) {
  window.__batchImportListenersAttached = true;

  document.addEventListener('click', async (e) => {
    // 1. グループ内の写真の「✕」ボタン（未所属プールへ移動）
